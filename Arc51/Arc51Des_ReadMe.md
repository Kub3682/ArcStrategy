# Arc51 C 程序代码对应的架构策略说明

回归系统架构设计层面：多核异构与数据驱动范式（系统级）
针对业务场景复杂化时选择不同控制范式策略，采用多控制器多节点网络通信，SOA架构（AUTOSAR CP--> AP）

> 应用场景变化：“LED 系统需支持多灯效并行：① 刹车灯（最高优先级，立即响应）；② 转向灯（中等优先级，50ms 精度）；③ 氛围灯（低优先级，100ms 精度）；④ 所有灯效互不阻塞，且刹车灯可打断其他灯效”。
> 用户场景说明：此时处于转向过程中，转向灯闪烁，同时氛围灯工作，然后用户紧急刹车，此时系统应立即响应刹车灯， “刹车灯触发→暂停转向灯→执行刹车灯逻辑”，整个过程氛围灯保持工作，但切换到另外一个灯效模式。
