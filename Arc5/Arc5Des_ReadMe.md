...
严格遵守开闭原则（尽可能的对扩展开放，对修改封闭，因为随着业务逻辑的变化，之前的抽象设计可能就不再适用，需要进一步抽象优化）：

- 开放：允许新增模块，新增功能，新增业务，以满足业务需求的变化。
- 封闭：不允许修改已有模块，功能，业务，以保证系统稳定性和可靠性。

1. MCAL层：增加Port配置（其他的LED灯），在PortCfg模块中；
2. Conf层：拓展豪华车型（LuxuryCarConfig）具体的配置项：
   - 配置刹车灯，灯效为：常灭，常亮；
   - 配置转向灯，灯效为：呼吸式闪烁，常灭；
   - 配置氛围灯，灯效为：循环流水式，整体频闪；
3. LED层：增加LEDEffectCfg抽象配置概念，增加灯效：呼吸式闪烁，常灭以及循环流水式。
4. 注意：本次需要调整之后的抽象设计，以满足业务需求的变化。
   - 原先的抽象设计中，CarModelConfig的抽象概念配置结构体粒度粗（仅activeEffect），直接关联单一灯效，导致无法满足不同配置多样性灯效的需求。
   - 本次需要优化设计为CarModelConfig的抽象，关联多个灯效，即LEDEffectCfg的抽象。
   - 进一步符合现实场景：什么车型对应有什么灯的配置，以及什么灯效的配置。

核心优化思路：把 “变化点” 完全抽象为 “可配置项”，让所有扩展仅需 “新增配置 / 新增模块”，无需修改任何已有代码
